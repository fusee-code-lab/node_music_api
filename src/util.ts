...
export function lyric_decode(str: string, needTranslate = false) {
  ...
  if (
    needTranslate &&
    translate_lyric_arr.filter((item: any) => item[1]).length === 0
  ) {
    translate_lyric_arr = [];
  }
  return needTranslate
...
export function completeCookie(cookie?: string) {
  ...
  if (!origin.includes("_ntes_nnid") || !origin.includes("_ntes_nuid")) {
    ...
    let nnid =
      randomString("0123456789abcdefghijklmnopqrstuvwxyz", 32) + "," + now;
    extra.push(["_ntes_nnid=" + nnid, "Expires=" + expire.toUTCString()]);
    extra.push([
      "_ntes_nuid=" + nnid.slice(0, 32),
      "Expires=" + expire.toUTCString(),
    ]);
  }
